-- ============================================================================
-- VUELTA ATRAS de `2026-09-07_trackeador_escalera_disponible.sql`. 7-sep-2026
-- ----------------------------------------------------------------------------
-- 🔴 PARA QUE EXISTE Y CUANDO SE LANZA. Un solo motivo: **que Elena deje de ver
--   filas en el Trackeador** despues de aplicar el paso 2c. Si eso pasa, esto se
--   lanza SIN CONSULTAR A NADIE: primero se devuelve la pantalla, luego se
--   investiga. Un almacen parado cuesta mas que una hora de diagnostico.
--
-- 🔑 QUE DESHACE: los quince cambios del 2c, y nada mas. La vista vuelve al estado
--   del 2b, texto incluido:
--     · se va el peldaño del medio (`stock_estimado`) y su agregado por ASIN;
--     · las seis decisiones vuelven al SUELO de dos peldaños
--       `COALESCE(stock_fba_eu, stock_vendible)`;
--     · el COMMENT de `stock_fba_eu` vuelve a decir lo que decia el 2b.
--   ⚠️ Lo que NO devuelve es el `COALESCE(fc_transfer, 0)` del 2a/2b: el nulo sigue
--   sin leerse como cero. Deshacer eso es otra vuelta atras, la del 2b, y tiene su
--   propio fichero (`2026-09-07_trackeador_nulo_vuelta_atras.sql`). Esta escalera
--   se puede quitar sin tocar aquello.
--
-- 🔒 COMO SE ESCRIBIO, que es lo que permite fiarse de ella: **no se ha vuelto a
--   copiar la vista a mano**. El cuerpo es, LITERAL, el texto que devuelve
--   `pg_get_viewdef('public.v_trackeador_pantalla', true)` en produccion el
--   7-sep-2026 —1.243 lineas, 46.643 caracteres, md5
--   d059326d6c53cdc0c3c39e7143408822—, y el fichero de ida se genero a partir de
--   ESE mismo texto. Una vuelta atras transcrita a mano puede traer su propio
--   error, y entonces no es una vuelta atras: es un tercer estado.
--
-- 🔴 NO TOCA `mv_trackeador_pantalla` (el 2c tampoco la toco, y recrear una
--   materializada la deja VACIA sin dar error), ni los permisos, ni la RLS, ni la
--   Guarda 12 del procesador, ni `inventario_fba.disponible_estimado`, que se
--   queda donde esta: la columna sigue rellenandose y la leen la pantalla de
--   reposicion y la de cobertura.
--
-- 🔴 `CREATE OR REPLACE VIEW` **BORRA LAS OPCIONES DE LA VISTA**, `security_invoker`
--   incluido. Lo cazo el numero de control del 2b en staging el 7-sep-2026, con
--   `reloptions: (sin opciones)` despues del REPLACE. Por eso hay un
--   `ALTER VIEW ... SET (security_invoker=true)` justo detras y la guarda lo mira.
--
-- 🔬 LO QUE PRUEBA LA GUARDA, y por que es sobre el TEXTO y no solo sobre los
--   datos: hoy el transito viene leido en las 381 fichas, asi que la escalera no
--   baja de peldaño y la version del 2c y esta dan LAS MISMAS CIFRAS. Una huella
--   de datos no distinguiria una de otra. Se comprueban las dos cosas: que el
--   texto vuelve (6 suelos de dos peldaños, 0 escaleras, la vista sin depender ya
--   de `disponible_estimado`) y que no se ha movido ni un dato.
-- ============================================================================

-- ── 0) LA FOTO DEL «ANTES» ──────────────────────────────────────────────────
-- 🔑 Ni una cifra de produccion escrita a mano: aqui tambien la invariante es
--    «revertir no mueve NINGUN dato», sea cual sea el entorno. Escribir 1.776 es
--    lo que hacia abortar los ensayos en staging, que devuelve 1.708.
...
-- 🔴 IMPRESCINDIBLE: el REPLACE se ha llevado las opciones de la vista.
ALTER VIEW public.v_trackeador_pantalla SET (security_invoker = true);

-- El COMMENT vuelve a decir lo que decia el 2b. Un comentario que sobrevive a la
-- vuelta atras es un comentario que MIENTE.
...
DO $$
DECLARE
  n int; huella text; tapones int; suelos int; escaleras int; usa_estimado int;
BEGIN
  -- 1 · 🔑 LA COMPROBACION DE VERDAD ES SOBRE EL TEXTO, no sobre los datos: hoy el
  --     transito se conoce en todas las fichas, asi que la version del 2c y esta
  --     dan LAS MISMAS CIFRAS y una huella de datos no las distinguiria.
  ...
  IF escaleras <> 0 THEN
    RAISE EXCEPTION 'ABORTA: quedan % escaleras de tres peldaños y tenian que irse '
                    'las 6. La vuelta atras no ha hecho su trabajo.', escaleras;
  END IF;
  IF suelos <> 6 THEN
    RAISE EXCEPTION 'ABORTA: han vuelto % suelos de dos peldaños y tenian que volver '
                    'los 6 del 2b.', suelos;
  END IF;
  IF tapones <> 0 THEN
    RAISE EXCEPTION 'ABORTA: ha vuelto el COALESCE que tapa el transito (% veces). '
                    'Esta vuelta atras es la del 2c, no la del 2b: el nulo tiene que '
                    'seguir sin leerse como cero.', tapones;
  END IF;

  -- 2 · Y POR ESTRUCTURA: la vista ya no depende de la columna del puente.
  ...
  IF usa_estimado <> 0 THEN
    RAISE EXCEPTION 'ABORTA: la vista sigue dependiendo de '
                    'inventario_fba.disponible_estimado. El peldaño no se ha ido.';
  END IF;

  -- 3 · Y no se ha movido ni un dato respecto a como estaba al empezar.
  ...
  IF (SELECT sum(t.stock_fba_eu) FROM public.v_trackeador_pantalla t)
     IS DISTINCT FROM (SELECT suma_stock_fba_eu FROM _pantalla_antes)
  OR (SELECT count(*) FROM public.v_trackeador_pantalla t WHERE t.accion = 'COMPROBAR')
     IS DISTINCT FROM (SELECT n_comprobar FROM _pantalla_antes) THEN
    RAISE EXCEPTION 'ABORTA: ha cambiado la suma de stock_fba_eu o el recuento de '
                    'COMPROBAR al revertir.';
  END IF;

  -- 4 · security_invoker, el SELECT y la materializada.
  IF (SELECT coalesce(array_to_string(reloptions, ','), '') FROM pg_class
       WHERE oid = 'public.v_trackeador_pantalla'::regclass) NOT LIKE '%security_invoker=true%' THEN
  ...
  RAISE NOTICE 'Numero de control OK: se han ido las 6 escaleras, han vuelto los 6 '
               'suelos, la vista ya no depende del disponible estimado, % filas sin '
               'mover un dato, security_invoker y el SELECT en su sitio y la '
               'materializada llena.', (SELECT filas FROM _pantalla_antes);
END $$;

-- ============================================================================
-- VERIFICACION POSTERIOR (por SQL, aparte del job):
--
--   select count(*) filter (where l like '%COALESCE(c.stock_fba_eu, c.stock_vendible%'
--                              or l like '%COALESCE(h.stock_fba_eu, h.stock_vendible%') as suelos_vueltos,
--          count(*) filter (where l like '%stock_fba_eu, %stock_estimado%stock_vendible%') as escaleras_que_quedan,
--          count(*) filter (where l like '%COALESCE(sf.uds_fc_transfer%')                as tapones
--     from unnest(string_to_array(
--            pg_get_viewdef('public.v_trackeador_pantalla'::regclass, true), chr(10))) as l;
--   -- → 6 · 0 · 0
--
