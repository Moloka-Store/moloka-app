-- ============================================================================
-- v_trackeador_pantalla — EL NULO DEJA DE LEERSE COMO CERO.        7-sep-2026
-- (paso 2b de los cuatro del plan; el 2a fue la rama de salud)
-- ----------------------------------------------------------------------------
-- POR QUÉ. Desde el 7-sep Amazon puede servir el informe FBA sin la columna del
--   tránsito, y `inventario_fba.fc_transfer` queda a NULO. Esta vista lo tapaba
--   con `COALESCE(..., 0)` en dos sitios, así que el «no lo sé» salía como CERO y
--   el stock FBA de la pantalla de Elena quedaría corto sin decirlo.
--
-- 🔴 CÓMO SE REVISA ESTE FICHERO, porque son 1.237 líneas y casi todas son
--   arrastre de columnas entre los nueve CTE encadenados. **El texto está copiado
--   LITERAL de `pg_get_viewdef('public.v_trackeador_pantalla', true)` de
--   producción del 7-sep-2026.** Sólo hay OCHO líneas cambiadas y todas llevan la
--   marca `⇐ CAMBIO`. Para revisarlo:
--       grep -n 'CAMBIO' migraciones/2026-09-07_trackeador_el_nulo_no_es_cero.sql
--   y para comprobar que lo demás no se ha tocado, diff contra el viewdef.
--
-- LOS OCHO CAMBIOS, en dos grupos:
--
--   GRUPO A · lo que se ENSEÑA deja de mentir (2 líneas)
--     · CAMBIO 1 — el agregado por ASIN del CTE `stock_fba`. Quitar el COALESCE no
--       basta: `sum()` **ignora los nulos en silencio**, así que un ASIN con dos
--       SKU y el tránsito leído en uno solo daría la suma parcial como total.
--       Probado en producción antes de escribirlo, con datos de mentira:
--           todo leído  → viejo 8 · nuevo 8     (no cambia nada)
--           uno falta   → viejo 5 · nuevo NULO  (el viejo mentía)
--           todo falta  → viejo 0 · nuevo NULO  (el viejo mentía)
--     · CAMBIO 2 — el `LEFT JOIN` del CTE `base`. Aquí el COALESCE tenía DOS
--       significados mezclados y sólo uno era correcto: «este ASIN no está en FBA»
--       (0 está bien) y «está, pero no se sabe su tránsito» (0 es mentira). Se
--       separan mirando si el LEFT JOIN casó. Probado igual:
--           en FBA, leído     → viejo 3 · nuevo 3
--           en FBA, sin dato  → viejo 0 · nuevo NULO
--           no está en FBA    → viejo 0 · nuevo 0
--     `stock_fba_eu` no se toca: hereda el nulo solo, porque `NULL + n` es NULL.
--
--   GRUPO B · lo que se DECIDE no cambia (6 líneas)
--     🔴 `stock_fba_eu` NO es sólo una cifra en pantalla: alimenta SEIS
--     decisiones (los veredictos COMPROBAR y LIQUIDA, `ficha_retirada`,
--     `reclamar la ficha` y el aviso de almacenaje). Medido sobre las 1.776 filas:
--     si el nulo llegara a esas decisiones, **737 fichas pasarían a COMPROBAR** el
--     día que faltase el tránsito, y 13 dejarían de marcarse como ficha retirada.
--     El motivo es que `GREATEST(NULL, 0)` devuelve **0** en Postgres: GREATEST y
--     LEAST ignoran los nulos, al revés que la aritmética. Un «no lo sé» que pasa
--     por ahí sale convertido en afirmación y no lo caza ninguna guarda.
--     Por eso las seis pasan a usar `COALESCE(stock_fba_eu, stock_vendible)`: el
--     vendible como SUELO. Con eso las decisiones son exactamente las de hoy.
--
--     ⚠️ ESE SUELO ES TEMPORAL Y TIENE SUSTITUTO: el paso 2c lo cambia por
--     `disponible_estimado` (el puente del paso 1). Cada una de las seis líneas lo
--     dice en su propio comentario, para que dentro de tres meses nadie lo
--     confunda con una decisión de diseño y no se atreva a tocarlo.
--
-- 🔒 QUÉ **NO** TOCA ESTE FICHERO:
--     · `mv_trackeador_pantalla`. No cambia ninguna columna, así que la
--       materializada no hace falta recrearla — y menos mal: una materializada
--       recreada **nace VACÍA**, sin dar error, y dejaría el Trackeador de Elena en
--       blanco hasta el REFRESH. Tampoco se le pone `security_invoker`: es una
--       materializada (`relkind='m'`) y no lo admite ni lo necesita.
--       Tampoco se refresca: hoy el tránsito viene leído en las 381 fichas, así que
--       lo que tiene guardado ya es idéntico a lo que produce la vista nueva. Lo
--       recogerá en su refresco de siempre.
--     · La Guarda 12 del procesador. La carga sigue CERRADA; se abre en el paso 4,
--       y 🔴 **el paso 4 no se hace hasta que el 2c esté dentro**: abrir con el
--       suelo puesto y sin el estimado daría reposiciones con un disponible corto.
--     · `security_invoker`, que esta vista ya tiene desde hace tiempo.
--
-- 🔬 LA PRUEBA QUE HACE SEGURA LA TRANSCRIPCIÓN. Hoy el tránsito se conoce en las
--   381 fichas, luego **la vista nueva tiene que producir EXACTAMENTE lo mismo que
--   la vieja**. El número de control compara la huella md5 de las 1.776 filas
--   enteras contra la de antes de tocar nada, medida en producción el 7-sep-2026:
--       filas 1.776 · huella e3391208f81387b4aa9ff704923acb2f
--       suma stock_fba_eu 26.768 · suma stock_fc_transfer 1.020
--   Si se me hubiera escapado un carácter al copiar, esa huella no cuadra y esto
--   ABORTA. Es lo que convierte 1.237 líneas copiadas a mano en algo verificable.
--
-- ⚠️ EL ENSAYO SOLO PRUEBA ALGO SI HAY ALGO QUE CAMBIAR. El testigo:
--       select count(*) from regexp_split_to_table(
--                pg_get_viewdef('public.v_trackeador_pantalla'::regclass, true), E'\n') l
--        where l like '%COALESCE(sf.uds_fc_transfer%';   -- 1 = hay algo que hacer
--
--
-- 🔬 Y LA TRANSCRIPCIÓN ESTÁ PROBADA BYTE A BYTE, no «revisada». Se generó la
--   vuelta atrás invirtiendo los ocho cambios de este fichero con un script, y su
--   texto resultó **idéntico** al que produce producción hoy:
--       longitud 46.179 caracteres · md5 944763a901b1931faff1f0e131b07ace
--   O sea que este fichero es el viewdef de producción MÁS los ocho cambios
...
CREATE OR REPLACE VIEW public.v_trackeador_pantalla AS
 WITH ven_pais AS (
         ...
         SELECT i.asin,
            ...
            -- ⇐ CAMBIO 1 · antes: sum(COALESCE(i.fc_transfer, 0)). Quitar el
            --   COALESCE no bastaba: sum() ignora los nulos en silencio y un ASIN
            --   con el transito leido en solo uno de sus SKU daria la suma parcial
            --   como si fuera el total. Si a alguno le falta, el ASIN no lo sabe.
            CASE WHEN count(*) = count(i.fc_transfer) THEN sum(i.fc_transfer) END AS uds_fc_transfer,
         ...
         SELECT n.asin,
            ...
            -- ⇐ CAMBIO 2 · antes: COALESCE(sf.uds_fc_transfer, 0::bigint). Ese
            --   COALESCE mezclaba dos cosas y solo una era correcta: «este ASIN no
            --   esta en FBA» (0 esta bien, el LEFT JOIN no caso) y «esta en FBA
            --   pero no se sabe su transito» (0 es mentira). Se separan mirando si
            --   el join caso, que es lo unico que las distingue.
            CASE WHEN sf.asin IS NULL THEN 0::bigint ELSE sf.uds_fc_transfer END AS stock_fc_transfer,
         ...
         SELECT c.asin,
            ...
            c.rival_flojo,
                ...
                CASE
                    ...
                    -- ⇐ CAMBIO 3 · SUELO TEMPORAL, lo sustituye el paso 2c por disponible_estimado.
                    WHEN c.presencia = 'SIN_PAGINA'::text AND (c.stock_moloka > 0::numeric OR COALESCE(c.stock_fba_eu, c.stock_vendible::numeric) > 0::numeric OR c.vendo_30d > 0) THEN ('tenías ficha y Amazon la retiró: '::text ||
                    CASE
                        -- ⇐ CAMBIO 4 · SUELO TEMPORAL, lo sustituye el paso 2c por disponible_estimado.
                        WHEN COALESCE(c.stock_fba_eu, c.stock_vendible::numeric) > 0::numeric THEN COALESCE(c.stock_fba_eu, c.stock_vendible::numeric) || ' uds en FBA pagando almacenaje'::text
                ...
                CASE
                    ...
                    -- ⇐ CAMBIO 5 · SUELO TEMPORAL, lo sustituye el paso 2c. 🔴 Aqui es
                    --   donde mas muerde: GREATEST(NULL, 0) devuelve 0 en Postgres, asi
                    --   que sin este COALESCE 737 de las 1.776 fichas pasarian a
                    --   COMPROBAR el dia que faltase el transito.
                    WHEN c.bb_precio IS NULL OR c.margen_al_bb IS NULL OR c.mi_precio IS NULL OR GREATEST(COALESCE(c.stock_fba_eu, c.stock_vendible::numeric), c.stock_moloka) = 0::numeric THEN 'COMPROBAR'::text
                    WHEN c.vendo_30d > 0 AND c.margen_hoy < 0::numeric THEN 'SUBE'::text
                    -- ⇐ CAMBIO 6 · SUELO TEMPORAL, lo sustituye el paso 2c por disponible_estimado.
                    WHEN c.vendo_30d_global = 0::numeric AND c.mi_precio > c.bb_precio AND COALESCE(c.stock_fba_eu, c.stock_vendible::numeric) > 0::numeric THEN 'LIQUIDA'::text
         ...
         SELECT h.asin,
            ...
            h.motivo_bloqueo_paneu IS NOT NULL AS es_bloqueo_amazon,
                CASE
                    ...
                    -- ⇐ CAMBIO 7 · SUELO TEMPORAL, lo sustituye el paso 2c por disponible_estimado.
                    WHEN h.presencia = 'SIN_PAGINA'::text AND (h.stock_moloka > 0::numeric OR COALESCE(h.stock_fba_eu, h.stock_vendible::numeric) > 0::numeric OR h.vendo_30d > 0) THEN 'ficha_retirada'::text
                ...
                CASE
                    ...
                    -- ⇐ CAMBIO 8 · SUELO TEMPORAL, lo sustituye el paso 2c por disponible_estimado.
                    WHEN h.presencia = 'SIN_PAGINA'::text AND (h.stock_moloka > 0::numeric OR COALESCE(h.stock_fba_eu, h.stock_vendible::numeric) > 0::numeric OR h.vendo_30d > 0) THEN 'reclamar la ficha'::text
...
COMMENT ON COLUMN public.v_trackeador_pantalla.stock_fc_transfer IS
  ...
  'trae esa columna: 🔴 NULO NO ES CERO. Antes se tapaba con COALESCE y el stock de '
  'la pantalla salia corto sin decirlo. 0 significa «este ASIN no esta en FBA».';
COMMENT ON COLUMN public.v_trackeador_pantalla.stock_fba_eu IS
  ...
  'usan COALESCE(stock_fba_eu, stock_vendible), un SUELO TEMPORAL que el paso 2c '
  'sustituye por disponible_estimado. Ver los CAMBIO 3..8 de '
  'migraciones/2026-09-07_trackeador_el_nulo_no_es_cero.sql.';
...
' ORDER BY t.asin, t.dominio))
  ...
  --     pero comprobarlo es gratis y el dia que no se cumpla hay que enterarse).
  ...
  -- 4 · 🔴 Y NO SE HA PERDIDO EL SELECT, que es lo que apaga la pantalla.
  ...
  IF has_table_privilege('authenticated', 'public.v_trackeador_pantalla', 'INSERT')
    ...
    RAISE EXCEPTION 'ABORTA: `authenticated` tiene escritura sobre la vista.';
  END IF;
...
--
--   -- los dos COALESCE que tapaban el nulo ya no estan, y los seis suelos si:
--   select count(*) filter (where l like '%COALESCE(sf.uds_fc_transfer%')  as tapones_viejos,
--          count(*) filter (where l like '%COALESCE(c.stock_fba_eu%'
--                              or l like '%COALESCE(h.stock_fba_eu%')      as suelos_nuevos
--     from regexp_split_to_table(
--            pg_get_viewdef('public.v_trackeador_pantalla'::regclass, true), E'\n') l;
--   -- → 0 · 6
--
--   -- y nada se ha movido (hoy el transito se conoce en todas las fichas):
